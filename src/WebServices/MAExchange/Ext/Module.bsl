Функция НовыйИдентификаторПодписчикаУведомлений(КодУзла, ИдентификаторXDTO)
	...
	УзелОбмена = ПланыОбмена.МобильныеУстройства.НайтиПоКоду(КодУзла);
	Если УзелОбмена.Пустая() Тогда
		ВызватьИсключение("Неизвестное устройство - " + КодУзла);
	КонецЕсли;
	Узел = УзелОбмена.ПолучитьОбъект();
	Узел.ИдентификаторПодписчикаДоставляемыхУведомлений = Новый
	ХранилищеЗначения(Идентификатор);
	Узел.Записать();
