...
Процедура ОтправитьСообщение(ИдЧата, ТекстСообщения, СообщениеСсылка) Экспорт
	...
	Иначе
		...
		ЗаписьЖурналаРегистрации("Telegram." + ИмяМетода, УровеньЖурналаРегистрации.Ошибка, , СообщениеСсылка, Результат.Получить("description")); 
...
Процедура ПолучитьНовыеСообщения() Экспорт
	...
	Иначе
		...
		ЗаписьЖурналаРегистрации("Telegram." + ИмяМетода, УровеньЖурналаРегистрации.Ошибка, , , Результат.Получить("description")); 
...
Процедура ОбработатьПолученныеСообщения(ПолученныеСообщения)
	...
	Для каждого КореньСообщения Из ПолученныеСообщения Цикл
		...
		ДатаСоздания	= '19700101' + ТелоСообщения.Получить("date") + (ТекущаяДата() - ТекущаяУниверсальнаяДата());
